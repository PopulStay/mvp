...
contract RentHouse {
  ...
  function setRentContracts(uint _uuid, string _doorkey,uint _price,uint _start,uint _end)
    public
    returns  (bool success)
  {
    rentContracts[_uuid] = RentContract(
      {
        doorkey : _doorkey,
        price   : _price,
        start   : _start,
  ...
  function getRentContract(uint _uuid)
    view
    public
    returns (uint _price,uint _start, uint _end, uint _contractdatetime,address _owner,uint _state)
  {
  ...
  function Rent(uint _uuid,address token)
    public
    returns (bool success)
  {
